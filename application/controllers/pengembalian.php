...
class Pengembalian extends CI_Controller{
    ...
    function cariTransaksi(){
        ...
        if($transaksi->num_rows()>0){
            ...
            echo $transaksi['nim']."|".$transaksi['tanggal_pinjam']."|".$transaksi['tanggal_kembali']."|".$transaksi['nama'];
        }
    ...
    function simpan(){
		...
		$this->m_pengembalian->bukuKembali($no);
        $this->m_pengembalian->update($no,$update);
        
        $this->m_pengembalian->simpan($info);
    }
    
    function cari_by_trans(){
        $no=$this->input->post('trans');
        $data['pencarian']=$this->m_pengembalian->cari_by_trans($no)->result();
