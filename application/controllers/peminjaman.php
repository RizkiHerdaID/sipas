...
class Peminjaman extends CI_Controller{
    ...
    function __construct(){
        ...
        $this->load->model(array('m_peminjaman','m_buku'));
    ...
    function sukses(){
        ...
        foreach($tmp as $row){
            ...
            $this->m_buku->update($row->kode_buku,$info);
    ...
    function cariAnggota(){
        $nim=$this->input->post('nim');
        $jmlPinjam = $this->m_peminjaman->count($nim)->row_array();
        $anggota=$this->m_peminjaman->cariAnggota($nim);
        if($anggota->num_rows()>0){
            $anggota=$anggota->row_array();
            echo $anggota['nama']."|".$anggota['hp']."|".$jmlPinjam['total'];
        }
    }
